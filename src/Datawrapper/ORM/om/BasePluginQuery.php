...
abstract class BasePluginQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'datawrapper', $modelName = 'Datawrapper\\ORM\\Plugin', $modelAlias = null)
    {
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   \Datawrapper\ORM\PluginDataQuery A secondary query class using the current class as primary query
     */
    public function usePluginDataQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinPluginData($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'PluginData', '\Datawrapper\ORM\PluginDataQuery');
    }
