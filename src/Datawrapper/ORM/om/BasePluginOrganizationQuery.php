...
use Datawrapper\ORM\PluginOrganizationPeer;
...
abstract class BasePluginOrganizationQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'datawrapper', $modelName = 'Datawrapper\\ORM\\PluginOrganization', $modelAlias = null)
    {
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   \Datawrapper\ORM\PluginQuery A secondary query class using the current class as primary query
     */
    public function usePluginQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinPlugin($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Plugin', '\Datawrapper\ORM\PluginQuery');
    }
