...
class HealthCheck {
    ...
    protected function checkConfig() {
        if (!file_exists(ROOT_PATH.'config.yaml')) {
            throw new \Exception(
                '<h2>Could not find <b>config.yaml</b></h2>'.
    ...
    protected function checkPathPermissions() {
        $paths = array();
        $rel   = ROOT_PATH;
        $err   = array();
    ...
    protected function checkDatabase() {
        if (!file_exists(ROOT_PATH.'/db/conf/datawrapper-conf.php')) {
            throw new \Exception(
                '<h2>No database configuration found!</h2>'.
        ...
        try {
            ...
            $this->pdo = new \PDO($dbconn['dsn'], $dbconn['user'], $dbconn['password']);
        }
        catch (\Exception $e) {
            throw new \Exception('<h2>Could not access database!</h2><p>'.htmlspecialchars($e->getMessage(), ENT_QUOTES, 'UTF-8').'</p>');
        }
        ...
        if (count($missingTables) > 0) {
            throw new \Exception(
                '<h2>Database is not initialized or corrupt</h2>'.
                ...
                '<p>Have you run the DB initialization in <code>db/sql/schema.sql</code>?</p>'
            );
        }
    }
    ...
    protected function checkPlugins() {
        $res     = $this->pdo->query('SELECT id FROM plugin WHERE enabled = 1');
        $missing = array();
        $cnt     = 0;
        ...
        if (count($package_json_parse_error) > 0) {
            throw new \Exception(
                '<h2>Some plugins have bad package descriptors</h2>'.
                ...
                '<ul><li><code>'.implode('</li></code><li><code>', $package_json_parse_error).'</code></li></ul>'
            );
        }

        if (count($need_newer_version) > 0) {
            throw new \Exception(
                '<h2>Some required plugins need to be updated</h2>'.
                '<p>The following plugins are installed but some plugins need a newer '.
                'version of them:</p>'.
                '<ul><li><code>'.implode('</li></code><li><code>', $need_newer_version).'</code></li></ul>'
            );
        }
        ...
        if (count($missing_dep) > 0) {
            throw new \Exception(
                ...
                '<ul><li><code>'.implode('</li></code><li><code>', $missing_dep).'</code></li></ul>'
            );
        }

        if ($cnt == 0) {
            throw new \Exception(
                '<h2>Please install some plugins</h2>'.
                ...
                'inside the Datawrapper directory.</p><pre><code>'.
